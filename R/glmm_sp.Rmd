...
```{r, eval=FALSE}
plot(p_sp, add.data = T,
     ci.style = "dot", line.size = 1, 
  ...
  geom_ribbon(aes(ymin =p_sp$conf.low, ymax=p_sp$conf.high), #both ribbon and
              alpha = .05, linetype = "dotted")
              
plot(p_sp, ci.style = "dot", line.size = 1, 
               colors = c("black","#d6604d","#377eb8") ) + #col =c("#8c510a","#d6604d","#377eb8")
   labs(title="", x="Time since deployment in days \ ", y="Detection rate") +
   ggpubr::theme_classic2() +
  scale_x_continuous(labels = c(0,20,40,60,80)) + #add scaled labels 
...
p_grid <- cowplot::plot_grid(p_dens, #cowplot:: must be specified, as sjPlot has "the same" function
                             jpg,                          #ie. masks the function
                             p_eq,
                             p_sp1,
                   rel_widths = c(3.8,3),
                   rel_heights = c(3,4),
...
```{r ekorn0, ref.label='sp', eval=TRUE}
