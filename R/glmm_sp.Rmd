...
plot(para_sp, show_intercept = TRUE) + labs(subtitle = ' standardize = "refit" ',
...
```{r model-report}
library(xtable)
para_raa  <- model_parameters(m_raa,   standardize = "refit", two_sd = TRUE, exponentiate = TRUE) %>% 
  add_row(Parameter = "Roe deer", .before = 1) # adds a row for species name in the resulting table
para_rev  <- model_parameters(m_rev,   standardize = "refit", two_sd = TRUE, exponentiate = TRUE) %>% 
  add_row(Parameter = "Red fox",  .before = 1)
para_grvl <- model_parameters(m_grvl,  standardize = "refit", two_sd = TRUE, exponentiate = TRUE) %>% 
  add_row(Parameter = "Badger",   .before = 1)
para_elg  <- model_parameters(m_elg,   standardize = "refit", two_sd = TRUE, exponentiate = TRUE) %>% 
  add_row(Parameter = "Moose",    .before = 1)
para_hjort<- model_parameters(m_hjort, standardize = "refit", two_sd = TRUE, exponentiate = TRUE) %>% 
  add_row(Parameter = "Red deer", .before = 1)
para_gaup <- model_parameters(m_gaup,  standardize = "refit", two_sd = TRUE, exponentiate = TRUE) %>% 
  add_row(Parameter = "Lynx",     .before = 1)
