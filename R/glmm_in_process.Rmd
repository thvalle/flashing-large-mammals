...
First, I filter out my species of interest.
...
# filter species
sp = "raadyr"
time_sp <- filter(time.dep2, species %in% sp) # .dep2 = without control
m  <- glmer(n.obs ~ time.deploy + as.factor(flash) + # fixed effects
...
```{r raadyr-61}
m61  <- glmer(n.obs ~ time.deploy + as.factor(flash) + # fixed effects
              (1 | loc) + (1 | month), # random effects
...
fit <- glmer(
  n.obs ~ time.deploy + as.factor(flash) + species + # fixed effects. Species included
    (1 | loc) + (1 | month),  # random effects
  data=time.dep2, # Control excluded ([time.dep2$time.deploy < 6.1,])
...
Or maybe cumulative counts?
```{r summing}
csum <- time.dep2 %>% group_by(loc, species, period, time.deploy) %>% 
  summarise(count = cumsum(n.obs)) #%>%
time.csum <- time.dep2 %>% left_join(csum)
time.csum$count %>% table()

# filter species
# sp = "raadyr"
# time_sp <- filter(time.cumsum, species %in% sp) # .dep2 = without control
# m  <- glmer(cumsum ~ time.deploy + as.factor(flash) + # fixed effects
#               (1 | loc) + (1 | month), # random effects
#             data   = time_sp,
#             family = poisson) # poisson family of distributions
# summary(m)
