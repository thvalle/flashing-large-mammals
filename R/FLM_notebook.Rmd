...
visits <- read_csv("visits.csv") %>% filter(!loc==664) #feil i dato-kolonner
field_work <- list()
field_work$v1 = ymd(range(visits$v_date1,na.rm = T))
field_work$v2 = ymd(range(visits$v_date2,na.rm = T))
field_work$v3 = ymd(range(visits$v_date3,na.rm = T))
field_work$v4 = ymd(range(visits$v_date4,na.rm = T))
field_work$v5 = ymd(range(visits$v_date5,na.rm = T))
field_work <- as_tibble(field_work)
# vector to make break-points for vline and bottom x-axis
...
rects <- data.frame(xstart = f_work[c(1,3,5,7,9)], xend = f_work[c(2,4,6,8,10)], field = "work")
```
...
p_eff +
  ...
  geom_rect(data = rects, aes(xmin = xstart, xmax = xend, ymin = -Inf, ymax = Inf),
            alpha = 0.2 ) #+
  # theme(legend.text = element_text("flash"))
```
