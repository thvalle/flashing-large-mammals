...
for(i in row.n){
  period<-obs[i,"period"]
  loc<-obs[i,"loc"]
  max.time<-max(effort[effort$loc%in%loc & effort$period%in%period,"date"]) # Setting the latest datetime as the end of the period, this account for when the camera was active
...
hist(obs[obs$flash==TRUE & obs$validated_species%in%sp & !obs$period%in%"Control" & obs$event==TRUE,]$t.diff) 
# Histogram of time to new detection in a period without flash
hist(obs[obs$flash==FALSE & obs$validated_species%in%sp & !obs$period%in%"Control" & obs$event==TRUE,]$t.diff)

# Histogram of time to new detection when animal was flashed
hist(obs[obs$flashed==TRUE & obs$validated_species%in%sp & !obs$period%in%"Control"  & obs$event==TRUE,]$t.diff) 
...
ggforest(mod0, data = obs[obs$validated_species%in%sp & !obs$period%in%"Control",])
...
mod3<-coxph(Surv(t.diff, event, type="right")~flashed*house_d2_ln + forestroad_d2_ln, data=obs[obs$validated_species%in%sp & !obs$period%in%"Control",])
summary(mod3)
