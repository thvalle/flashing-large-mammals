...
## Based on the tidymodels article 5 - case study
...
obs_test  %>% 
  ...
  pander()
```
Splitting with white LED present/not present resulted in a reasonably identical proportion of events in both groups.
...
```{r}
lr_recipe <- 
  recipe(event ~ ., data = obs_other) %>% 
  step_rm(date, datetime, timeserie_id, ) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>% 
  step_zv(all_predictors()) %>% 
  step_normalize(all_predictors())
```
"timeserie_id"  "datetime"   
      "ID"            
