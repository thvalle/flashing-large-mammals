...
overlap_flash <- function(x, colour = c("black", "blue")) {
  x1 <- obs[obs$validated_species %in% x & obs$flash == TRUE, ]$rad # species when flash = TRUE
  x0 <- obs[obs$validated_species %in% x & obs$flash == FALSE, ]$rad #   - - | | - -     = FALSE
  overlapPlot(x1, x0, main = str_to_title(x), linecol = rep(colour, 2), linewidth = c(2, 2), olapcol = "darkgrey", extend = "lightgrey") # plot
  x.est <- round(overlapEst(x1, x0, type = "Dhat4"), 3) # estimated overlap using Dhat4
  legend("top", legend = str_c("Dhat4", x.est, sep = " = "), bty = "n") # Dhat4 estimate label
  legend("bottomleft", legend = c("Flash", "No flash"), col = colour, lty = c(1, 2), lwd = 2, bty = "n") # legend
}
overlap_flash_2sp <- function(x, y, col_x, col_y) {
  ...
  overlap_flash(x, col_x) # using the overlap_flash function for single species w/w/o flash-plot
  ...
  legend("bottomleft", legend = c(x, y), col = c(col_x, col_y), lty = c(1, 1), lwd = 2, bty = "n") # legend
...
The second plots shows the effort from each camera, faceted by the periods. Unfortunately there is some debugging to do as _a camera from group B and one from C has fallen into the control period._
...
Roe deer and red fox are the species with decidedly most data. The flash true/false-plot reveals similar proportions of detection abundances.
...
sp <- c("rev", "raadyr", "hare", "grevling") # Four species with the most datapoints
...
```{r some_remaining_species_boxplot}
freq %>% # plot of frequency grouped by abc and flash T/F
  filter(validated_species %in% c("gaupe", "ekorn", "elg", "hjort", "maar", "katt")) %>%
  left_join(stations, by = "loc") %>%
  ggplot(aes(abc, freq)) +
  facet_wrap(~validated_species, scales = "free", nrow = 2) + # scales = "free"
  labs(title = "Frequencies of species w/ | w/o flash") +
